...
export default function Classes() {
  ...
  const handleCreateClass = async (e) => {
    ...
    try {
      const classRef = await addDoc(collection(db, 'classes'), {
        name: newClassName.trim(), code, createdBy: user.uid, createdAt: serverTimestamp()
      });
  ...
  const handleJoinByCode = async (e) => {
    ...
    try {
      ...
      if (!existing.empty) { setJoinError('آپ پہلے سے اس درجے کے ممبر ہیں۔'); return; }
  ...
  const handleDeleteClass = async (id, name) => {
    ...
    if (!window.confirm(`"${name}" کو مکمل طور پر حذف کریں؟`)) return;
  ...
  if (!user) {
    return (
      <div className="glass-panel text-center animate-fade-in my-8 py-16">
        ...
        <h2 style={{ fontSize: '2rem' }}>درجات دیکھنے کے لیے لاگ ان کریں</h2>
  ...
  return (
    <div className="animate-fade-in delay-1">
      <div className="flex justify-between items-center my-8">
        <h2 style={{ fontSize: '2.5rem', margin: 0 }}>میرے درجات</h2>
        {isAdmin && (
          <button onClick={() => setShowCreate(!showCreate)} className="btn btn-primary">
            <Plus size={20} /> نیا درجہ بنائیں
          </button>
        )}
      </div>
      ...
      {activeClassName && (
        <div className="glass-panel mb-6 flex justify-between items-center" style={{ border: '2px solid var(--primary-color)', padding: '1rem 1.5rem' }}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-muted" style={{ margin: 0, fontSize: '1rem' }}>فعال درجہ</p>
      ...
      {showCreate && isAdmin && (
        <div className="glass-panel mb-6 animate-fade-in" style={{ border: '2px dashed var(--primary-color)' }}>
          <h3 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>نیا درجہ بنائیں</h3>
          <form onSubmit={handleCreateClass} className="flex gap-4" style={{ flexWrap: 'wrap' }}>
            <input type="text" placeholder="درجے کا نام مثلاً: جماعت دہم الف" className="input-field" style={{ flex: '1 1 250px' }} value={newClassName} onChange={e => setNewClassName(e.target.value)} required />
      ...
      <div className="glass-panel mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 style={{ fontSize: '1.5rem', margin: 0 }}>کوڈ سے درجہ جوائن کریں</h3>
        </div>
        <form onSubmit={handleJoinByCode} className="flex gap-4" style={{ flexWrap: 'wrap' }}>
          <input type="text" placeholder="کوڈ درج کریں مثلاً: DRJ-AB12" className="input-field" style={{ flex: '1 1 200px', textTransform: 'uppercase' }} value={joinCode} onChange={e => setJoinCode(e.target.value)} required />
          <button type="submit" className="btn" style={{ background: 'var(--secondary-color)', color: '#fff' }}>شامل ہوں</button>
        </form>
        {joinError && <p style={{ color: 'var(--danger)', marginTop: '1rem', fontSize: '1.1rem' }}>{joinError}</p>}
      </div>

      <h3 style={{ fontSize: '1.8rem', marginBottom: '1.5rem' }}>میرے درجے</h3>
      {loading ? (
          ...
          {myClasses.map(cls => (
            <div key={cls.id} className="glass-panel" style={{ border: activeClassId === cls.id ? '2px solid var(--primary-color)' : '1px solid var(--glass-border)' }}>
              <div className="flex justify-between items-start mb-4">
                ...
                {isSuperAdmin && (
                  <button onClick={() => handleDeleteClass(cls.id, cls.name)} title="درجہ حذف کریں"
              ...
              <button onClick={() => switchClass(cls.id, cls.name)} className="btn"
                ...
                {activeClassId === cls.id ? '✓ فعال درجہ' : 'اس درجے میں جائیں'}
              </button>
            </div>
          ))}
          {myClasses.length === 0 && (
            <p className="text-muted" style={{ gridColumn: '1 / -1', textAlign: 'center', fontSize: '1.2rem', paddingTop: '2rem' }}>
              آپ کسی بھی درجے کے ممبر نہیں ہیں۔ اوپر دیے گئے کوڈ سے شامل ہوں۔
            </p>
          )}
        </div>
      )}
