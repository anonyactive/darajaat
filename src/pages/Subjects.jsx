...
import { Book, Plus, Trash2, Edit, FileText, Lock } from 'lucide-react';
...
export default function Subjects() {
  ...
  const fetchSubjects = async () => {
    // Graceful check for blank Firebase configurations so app doesn't immediately crash.
    if (!auth.app.options.apiKey || auth.app.options.apiKey === 'YOUR_API_KEY') {
        setSubjects([
            { id: 1, name: 'قرآن مجید (تفسیر) - (فرضی ڈیٹا)', booksCount: 2, totalPages: 500, readPages: 225 },
        ]);
        setLoading(false);
        return;
    }
  ...
  const handleAddSubject = async (e) => {
    ...
    try {
      ...
      alert("Error adding subject. Checking permissions.");
    }
  };
    ...
    <div className="animate-fade-in delay-2">
      ...
      {showAddForm && isAdmin && (
        <div className="glass-panel mb-8 animate-fade-in" style={{ border: '2px dashed var(--primary-color)' }}>
          <h3 className="mb-4 text-xl">نیا مضمون شامل کریں (صرف ایڈمن)</h3>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '2rem' }}>
                ...
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {isAdmin && (
                      <div className="flex gap-4">
                        ...
                        <button onClick={() => handleDelete(sub.id)} style={{ background: 'transparent', border: 'none', color: 'var(--danger)', cursor: 'pointer', transition: 'color 0.2s' }}><Trash2 size={20} /></button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-4 mt-4 mb-6">
                    <span className="text-muted flex items-center gap-2" style={{ fontSize: '1rem', background: 'rgba(255,255,255,0.05)', padding: '0.2rem 0.8rem', borderRadius: '20px' }}>
                      <Book size={16} /> {sub.booksCount || sub.books || 0} کتب
                    </span>
                    <span className="text-muted flex items-center gap-2" style={{ fontSize: '1rem', background: 'rgba(255,255,255,0.05)', padding: '0.2rem 0.8rem', borderRadius: '20px' }}>
          ...
          {subjects.length === 0 && <p className="text-muted text-center w-full mt-8" style={{ gridColumn: '1 / -1', fontSize: '1.2rem' }}>کوئی مضمون موجود نہیں۔ ابھی شامل کریں!</p>}
        </div>
      )}
    </div>
  );
